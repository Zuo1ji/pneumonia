...
class pneumonia:
    def __init__(self, targetProvinceName, additionProvinceName, gotProvinceName, additionMapName, mapname="map", virusname="virus"):
        ...
        with open(os.path.join('demo','static','cityname.json'),'r', encoding='UTF-8') as f1:
            self.cities=json.load(f1)
        with open(os.path.join('demo','static','country.json'),'r', encoding='UTF-8') as f2:
    ...
    def getWorldMap(self,count,title):     
        chinaConfirmCount=0
        for province in self.jsondata:
            chinaConfirmCount += province.get(count)
